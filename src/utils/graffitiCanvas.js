 ...
 *	Apache License Version 2.0,
	...
	Copyright 2017-2019 eguid(eguid@outlook.com)
...
export function GraffitiCanvas() {
  // 公共方法
  var getDom = function(id) { return document.getElementById(id) }; var // 获取dom
    isNull = function(s) { return s == undefined || typeof (s) === 'undefined' || s == null || s == 'null' || s === '' || s.length < 1 }; var // 是否为null或kong或underfind
    isTure = function(s) { return s == true || s == 'true' }; var // 是否为true
  ...
  var cursors = ['crosshair', 'pointer']// 鼠标图形
  // 全局变量
  var cbtCanvas; var /** 绘图容器*/
    cxt; var /** 绘图对象*/
    paintConfig = {// 绘图基础配置
  ...
  var Line = function(p1, p2, al) {
    var start = p1; var end = p2; var angle = al
    ...
    var drawArrow = function() {
      ...
      var x1 = p1.getX(); var y1 = p1.getY(); var x2 = p2.getX(); var y2 = p2.getY()
      var el = 50; var al = 15
      // 计算箭头底边两个点（开始点，结束点，两边角度，箭头角度）
      vertex[0] = x1, vertex[1] = y1, vertex[6] = x2, vertex[7] = y2
      // 计算起点坐标与X轴之间的夹角角度值
      var angle = Math.atan2(y2 - y1, x2 - x1) / Math.PI * 180
      var x = x2 - x1; var y = y2 - y1; var length = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2))
      if (length < 250) {
        el /= 2, al / 2
      } else if (length < 500) {
  ...
  var Circle = function(arr) {
    // 包含起始点（圆心）和结束点,以及圆半径
    var startPoint = arr.start; var endPoint = arr.end; var radius = arr.radius
    /* 绘制圆*/
  ...
  var setCuPointXY = function(x, y, i) {
    ...
    console.log('getCuPoint(i)--', getCuPoint(i), 'x, y, i---', x, y, i)
    return getCuPoint(i)
  }
  /** 保存一个恢复点*/
  var save = function() {
  ...
  var mouseMove = function(e) {
    if (isDrawing() && hasStartPoint()) { // 检查是否开始绘制，检查是否有开始坐标点
      ...
      switch (ctrlConfig.kind) {
          ...
          var poly = getCuGraph()
          console.log('react--point---', [lu, ru, p, ld], 'left--', lu.getX(), p.getY())
          poly.set([lu, ru, p, ld])
          poly.draw()
          break
			 case graphkind.circle:// 圆
  ...
  return {
    ...
    init: function(params) {
      ...
      if (cbtCanvas.getContext) {
        console.log('cbtCanvas---', cbtCanvas)
        /** 绘图对象*/
